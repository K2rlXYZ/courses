...
def find_capital_letters(s: str) -> str:
    ...
    """
    return "".join([x for x in s if x in "ABCDEFGHIJKLMNOPQRSTUVWXYZÕÄÜÖ"])
...
def close_far(a: int, b: int, c: int) -> bool:
    ...
    """
    if ((not b - a > 1) and (not b - a < -1) and (c - a >= 2 or c - a <= -2) and (c - b >= 2 or c - b <= -2)) or \
            ((not c - a > 1) and (not c - a < -1) and (b - a >= 2 or b - a <= -2) and (b - c >= 2 or b - c <= -2)):
        return True
    return False
...
def get_names_from_results(results_string: str, min_result: int) -> list:
    ...
    results = [[x[0:-1], int(x[-1])] for x in results]
    results = [x for x in results if x[-1] >= min_result]
    results = [(" ".join(x[0][0:-1]) + " " + x[0][-1]).strip() for x in results]
    return results
...
def tic_tac_toe(game: list) -> int:
    ...
    """
    for x in range(3):
        if game[x][0] == game[x][1] == game[x][2] and not game[x][0] == 0:
            return game[x][0]
        if game[0][x] == game[1][x] == game[2][x] and not game[0][x] == 0:
            return game[0][x]
    if game[0][0] == game[1][1] == game[2][2] and not game[0][0] == 0:
        return game[0][0]
    if game[0][2] == game[1][1] == game[2][0] and not game[0][2] == 0:
        return game[0][2]
    return 0


def rainbows(field: str, ind=0, str_ind=0, looking=0) -> int:
    """
    Count rainbows.
    ...
    reversed_rainbow = list("wobniar")
    if not field.islower():
        return rainbows(field.lower())

    if ind == len(field):
        return 0

    if str_ind == 6 and (field[ind] == rainbow[str_ind] or field[ind] == reversed_rainbow[str_ind]):
        return 1 + rainbows(field, ind + 1, 0, 0)

    if looking == 1 and field[ind] == rainbow[str_ind]:
        return rainbows(field, ind + 1, str_ind + 1, 1)
    if looking == 2 and field[ind] == reversed_rainbow[str_ind]:
        return rainbows(field, ind + 1, str_ind + 1, 2)

    if field[ind] == rainbow[0]:
        return rainbows(field, ind + 1, str_ind + 1, 1)
    elif field[ind] == reversed_rainbow[0]:
        return rainbows(field, ind + 1, str_ind + 1, 2)
    else:
        return rainbows(field, ind + 1, 0, 0)


def longest_substring(text: str, ind=0) -> str:
    """
    ...
    ind = 0
    while ind < len(text):
        if text[ind].lower() not in active_part.lower():
            active_part += text[ind]
        else:
            char = text[ind].lower()
            ind -= (len(active_part) - active_part.lower().index(char))-1
            active_part = active_part[0:active_part.lower().index(char)+1]
...
if __name__ == '__main__':
    print(find_capital_letters("abceDKGTOGH"))
    ...
    print("tic", tic_tac_toe([[0, 0, 0], [1, 1, 1], [0, 0, 0]]))
    print(rainbows("rainbowThisIsJustSomeNoise"))
    print(rainbows("WoBniar"))
    print(rainbows("rainbowobniar"))
    print(longest_substring("y\"MBZU:2B{uDW[]t<m	oY09'kP0%tE[a(gzY/rBR#bu *jC-\!>|V\JwO&~[/DQ3BH\\3SuRsImg\\"))
    hotel = Hotel()
    room1 = Room(1, 100)
