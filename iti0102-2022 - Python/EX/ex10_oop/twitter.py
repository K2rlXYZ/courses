...
def sort_by_popularity(tweets: list) -> list:
    ...
    """
    twets = sorted(tweets, key=lambda x: x.time)
    return sorted(twets, key=lambda x: x.retweets, reverse=True)
...
def filter_by_hashtag(tweets: list, hashtag: str) -> list:
    ...
    """
    twets = set([x if hashtag in x.content else None for x in tweets])
    twets = list(twets)
    twets.remove(None)
    return twets
...
def sort_hashtags_by_popularity(tweets: list) -> list:
    ...
    hashtags = {}
    for tweet in tweets:
        for x in re.findall("(#[A-Za-z]+)", tweet.content):
            if x not in hashtags.keys():
                hashtags[x] = 0
            hashtags[x] += tweet.retweets

    hashtag_list = list(hashtags.keys())
    hashtag_list.sort(key=lambda x: hashtags[x], reverse=True)
    return hashtag_list
