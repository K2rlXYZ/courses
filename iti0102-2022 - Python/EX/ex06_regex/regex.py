...
def find_words(text: str) -> list:
    ...
    """
    final = []
    for x in re.findall(r"[ÕÜÄÖA-Z][õüäöa-z]+", text):
        final.append(x)
    return final
...
def find_words_with_vowels(text: str) -> list:
    ...
    """
    final = []
    for x in re.findall(r"[AEIOUÕÜÄÖ][a-z]+", text):
        final.append(x)
    return final
...
def find_sentences(text: str) -> list:
    ...
    """
    final = []
    for x in re.findall(r"(([A-ZÄÕÖÜ][A-ZÄÕÖÜa-zäõüö \d\-,:]+)(\.+|\?+|!+))", text):
        final.append(x[0])
    return final
...
def find_words_from_sentence(sentence: str) -> list:
    ...
    """
    final = []
    for x in re.findall(r"[ÕÜÄÖõüäöA-Za-z\d]+", sentence):
        if x != '':
            final.append("".join(x))
    return final
...
def find_words_from_sentences_only(text: str) -> list:
    ...
    """
    senctences = find_sentences(text)
    print(senctences)
    final = []
    for sentence in senctences:
...
def find_phone_numbers(text: str) -> dict:
    ...
    """
    dic = {}
    for x in re.findall(r"(\+\d\d\d|)(\s|)((\d\d\d\d\d\d\d)|(\d\d\d\d\d\d\d\d))(?!\d)", text):
        if x is not None:
            if x[0].strip() != '':
                if x[0] not in dic.keys():
                    dic[x[0].strip()] = []
                dic[x[0].strip()].append(x[2])
            else:
                if '' not in dic.keys():
                    dic[''] = []
                dic[''].append(x[2])
...
if __name__ == '__main__':
    ...
    print(find_sentences(
        'see ei ole lause. Aga kas12 - ... see on? jah, oli.'))  # ['See on esimene - lause.', 'See on ä teine lause!', 'Aga kas see on?']
    ...
    print(find_sentences("Esimene lause... Teine lause... Kolmas lause..."))
